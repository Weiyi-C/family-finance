...
const form = reactive({ type: 'expense', amountYuan: 0, frequency: 'monthly', start_date: '', merchant_name: '', note: '' })
...
function editItem(row: RecurringTransaction) {
  ...
  form.frequency = row.frequency; form.start_date = row.start_date
  form.merchant_name = row.merchant_name || ''; form.note = row.note || ''
  showDialog.value = true
}

async function handleSave() {
  if (!form.amountYuan || !form.start_date) { ElMessage.warning('请填写金额和开始日期'); return }
  saving.value = true
  try {
    const payload = { type: form.type, amount: Math.round(form.amountYuan * 100), frequency: form.frequency, start_date: form.start_date, merchant_name: form.merchant_name || undefined, note: form.note || undefined }
    if (editingId.value) { await updateRecurring(editingId.value, payload); ElMessage.success('更新成功') }
    else { await createRecurring(payload); ElMessage.success('创建成功') }
