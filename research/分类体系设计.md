# 家庭记账系统 — 三级分类体系设计

> 设计原则：
> 1. 三级分类，从粗到细：一级→二级→三级
> 2. 覆盖中国家庭日常生活的方方面面
> 3. 每个分类有图标和颜色，方便识别
> 4. 系统预设 + 家庭可自定义扩展
> 5. 分类可归档（不用了但保留历史数据）

---

## 完整分类树

### 🍽️ 餐饮

| 二级分类 | 三级分类 |
|---------|---------|
| ☀️ 早饭 | 食堂、外卖、自己做、便利店、路边摊 |
| 🌞 午饭 | 食堂、外卖、自己做、堂食、便利店、快餐 |
| 🌙 晚饭 | 食堂、外卖、自己做、堂食、快餐 |
| 🫖 下午茶 | 奶茶、咖啡、甜点、果汁、茶饮 |
| 🌃 夜宵 | 烧烤、外卖、泡面、便利店 |
| 🍜 正餐聚餐 | 中餐、西餐、日料、韩餐、火锅、烧烤、自助餐 |
| 🥤 饮品 | 奶茶、咖啡、果汁、矿泉水、碳酸饮料、茶 |
| 🍰 甜点零食 | 蛋糕、面包、饼干、糖果、坚果、薯片、巧克力 |
| 🍎 水果 | 苹果、香蕉、葡萄、西瓜、车厘子、其他水果 |
| 🛒 食材采购 | 蔬菜、肉类、海鲜、蛋奶、粮油、调味品、半成品 |
| 🍱 工作餐 | 公司食堂、外卖工作餐、自带便当 |

### 🚗 交通出行

| 二级分类 | 三级分类 |
|---------|---------|
| 🚕 打车 | 滴滴、高德打车、曹操出行、T3出行、出租车 |
| 🚇 公共交通 | 地铁、公交、轮渡、城际铁路 |
| 🚄 火车高铁 | 高铁、动车、普通火车、城际 |
| ✈️ 飞机 | 国内机票、国际机票、机场建设费、燃油附加费 |
| 🚌 长途汽车 | 大巴、拼车 |
| 🚗 自驾加油 | 中石化、中石油、壳牌、其他加油站 |
| 🅿️ 停车费 | 路边停车、停车场、小区停车 |
| 🛵 共享单车 | 美团单车、哈啰单车、青桔单车 |
| 🔧 车辆保养 | 洗车、保养、维修、轮胎、电瓶 |
| 🚘 车险 | 交强险、商业险 |
| 📋 车辆费用 | 年检、违章罚款、ETC充值、过路费 |
| 🛤️ 出行服务 | 代驾、租车、充电桩 |

### 👕 服饰美妆

| 二级分类 | 三级分类 |
|---------|---------|
| 👔 男装 | T恤、衬衫、裤子、外套、西装、内衣 |
| 👗 女装 | 上衣、裙子、裤子、外套、内衣 |
| 👟 鞋子 | 运动鞋、皮鞋、凉鞋、拖鞋、靴子 |
| 👜 包包 | 双肩包、手提包、钱包、行李箱 |
| 💍 饰品 | 项链、手链、戒指、耳环、手表 |
| 💄 美妆护肤 | 护肤品、化妆品、香水、面膜、防晒 |
| 💇 美容美发 | 理发、染发、烫发、美甲、美容院 |
| 🧴 日用品 | 洗发水、沐浴露、牙膏、纸巾、洗衣液 |

### 🏠 住房

| 二级分类 | 三级分类 |
|---------|---------|
| 🏷️ 房租/房贷 | 月租、房贷还款 |
| 💡 水电燃气 | 电费、水费、燃气费 |
| 📶 通讯网络 | 手机话费、宽带费、IPTV |
| 🏠 物业费 | 物业管理费、停车管理费 |
| 🔧 维修维护 | 家电维修、管道疏通、开锁、装修维修 |
| 🛋️ 家居用品 | 床品、厨具、收纳、清洁工具、装饰品 |
| 🧹 家政服务 | 保洁、保姆、月嫂、搬家 |
| 🏗️ 装修 | 设计费、建材、家具、家电、人工费 |

### 🎮 休闲娱乐

| 二级分类 | 三级分类 |
|---------|---------|
| 🎬 影视 | 电影票、视频会员（爱奇艺/腾讯/B站）、演出票 |
| 🎵 音乐 | 音乐会员、演唱会、乐器 |
| 📚 阅读 | 书籍、电子书、Kindle会员、杂志 |
| 🎮 游戏 | 游戏充值、游戏购买、游戏道具、主机游戏 |
| 🏃 运动健身 | 健身房、瑜伽、游泳、球类运动、运动装备 |
| 🎳 休闲活动 | KTV、桌游、密室逃脱、剧本杀、棋牌 |
| 🎪 旅游景点 | 门票、导游、纪念品 |
| 📷 摄影 | 相机设备、冲印、摄影服务 |
| 🎨 兴趣爱好 | 手工、绘画、模型、收藏 |

### 💊 医疗健康

| 二级分类 | 三级分类 |
|---------|---------|
| 🏥 就医 | 门诊费、检查费、化验费、手术费、住院费 |
| 💊 药品 | 处方药、非处方药、保健品、维生素 |
| 🦷 口腔 | 洗牙、补牙、矫正、种牙 |
| 👁️ 眼科 | 眼镜、隐形眼镜、眼药水、视力检查 |
| 💉 疫苗体检 | 年度体检、疫苗接种、专项检查 |
| 🧘 保健调理 | 按摩、推拿、艾灸、中医调理 |
| 🏋️ 健身器材 | 哑铃、瑜伽垫、跑步机 |

### 📚 教育学习

| 二级分类 | 三级分类 |
|---------|---------|
| 📖 学费 | 学校学费、培训班学费 |
| 📝 考试考证 | 报名费、教材费、考试培训 |
| 📓 文具用品 | 笔、本子、文具、书包 |
| 💻 在线课程 | 网课、知识付费、付费专栏 |
| 🎓 辅导培训 | 课外辅导、兴趣班、家教 |
| 📖 教材教辅 | 课本、辅导书、练习册 |

### 🚗 养车（如果和交通分开）

| 二级分类 | 三级分类 |
|---------|---------|
| ⛽ 加油 | 中石化、中石油、其他 |
| 🔧 保养维修 | 常规保养、大修、轮胎、刹车片 |
| 🚿 洗车美容 | 普通洗车、精洗、打蜡、镀晶 |
| 🅿️ 停车 | 路边停车、停车场、小区月卡 |
| 🛣️ 过路费 | ETC、高速费、过桥费 |
| 📋 车辆行政 | 年检、违章、驾照相关 |
| 🛡️ 车险 | 交强险、商业险、驾乘险 |

### 👨‍👩‍👧 人情往来

| 二级分类 | 三级分类 |
|---------|---------|
| 🎁 礼物 | 生日礼物、节日礼物、伴手礼 |
| 💰 红包 | 微信红包、支付宝红包、现金红包 |
| 🎊 婚礼 | 随礼、份子钱 |
| 🎂 生日 | 生日聚餐、生日蛋糕、生日礼物 |
| 🏮 节日 | 春节、中秋、端午、圣诞、情人节 |
| 🤝 请客 | 请吃饭、请喝奶茶、商务宴请 |
| 💐 探望 | 探病、看望长辈、乔迁礼 |

### 👶 母婴亲子

| 二级分类 | 三级分类 |
|---------|---------|
| 🍼 奶粉辅食 | 婴儿奶粉、辅食、零食 |
| 🧸 玩具 | 益智玩具、户外玩具、电子玩具 |
| 👶 婴儿用品 | 纸尿裤、奶瓶、婴儿车、安全座椅 |
| 👗 童装 | 衣服、鞋子、帽子 |
| 📚 早教 | 早教班、绘本、早教玩具 |
| 🎠 亲子活动 | 游乐场、动物园、亲子旅行 |

### 🐾 宠物

| 二级分类 | 三级分类 |
|---------|---------|
| 🍖 宠物食品 | 狗粮、猫粮、零食、罐头 |
| 🏥 宠物医疗 | 疫苗、体检、驱虫、看病 |
| 🧴 宠物用品 | 玩具、猫砂、牵引绳、窝 |
| ✂️ 宠物服务 | 洗澡美容、寄养、训练 |
| 🐕 宠物其他 | 宠物保险、领养费 |

### 📱 数码科技

| 二级分类 | 三级分类 |
|---------|---------|
| 📱 手机 | 新手机、手机壳、贴膜、维修 |
| 💻 电脑 | 笔记本、台式机、配件、维修 |
| 🎧 数码配件 | 耳机、充电器、数据线、移动电源 |
| 🖨️ 办公设备 | 打印机、显示器、键盘鼠标 |
| 📺 家电 | 电视、冰箱、洗衣机、空调、小家电 |
| 🎮 游戏设备 | 游戏机、手柄、游戏光盘 |

### 💰 金融理财

| 二级分类 | 三级分类 |
|---------|---------|
| 💳 银行费用 | 年费、手续费、转账费、ATM |
| 📈 投资 | 基金、股票、理财、定期存款 |
| 🏦 贷款利息 | 房贷利息、车贷利息、信用贷利息 |
| 💱 汇率换汇 | 外币兑换、跨境汇款 |
| 📊 保险 | 人寿保险、重疾险、意外险、医疗险 |
| 💸 税费 | 个人所得税、房产税 |

### 🧳 差旅

| 二级分类 | 三级分类 |
|---------|---------|
| 🏨 住宿 | 酒店、民宿、青旅 |
| 🍽️ 差旅餐饮 | 出差餐费、招待餐费 |
| 🚕 差旅交通 | 出差打车、出差机票、出差火车 |
| 📋 差旅其他 | 签证费、行李寄存、机场贵宾厅 |

### 💼 工作办公

| 二级分类 | 三级分类 |
|---------|---------|
| 🖊️ 办公用品 | 文具、打印纸、墨盒 |
| 🍽️ 工作餐 | 工作日午餐、加班餐 |
| 🚕 工作出行 | 通勤打车、通勤地铁 |
| 📱 通讯办公 | 工作手机、办公软件订阅 |
| 🎓 职业发展 | 培训费、考证费、行业会议 |

### 🎁 其他

| 二级分类 | 三级分类 |
|---------|---------|
| ❓ 未分类 | 其他支出 |
| 🔄 转账 | 账户间转账、借钱、还钱 |
| 💸 手续费 | 转账手续费、提现手续费 |
| 🎰 彩票 | 彩票购买 |
| 🙏 慈善捐赠 | 公益捐款、慈善 |
| ⚖️ 罚款 | 交通罚款、其他罚款 |
| 📦 快递物流 | 快递费、运费 |

---

## 收入分类

| 一级分类 | 二级分类 | 三级分类 |
|---------|---------|---------|
| 💰 工资薪酬 | 月薪、年终奖、绩效奖金、加班费、补贴津贴 |
| 💵 副业收入 | 兼职、 freelance、稿费、咨询费 |
| 🎁 红包收入 | 微信红包、支付宝红包、现金红包 |
| 💸 转账收入 | 借钱收回、转账收款 |
| 📈 投资收益 | 基金收益、股票收益、利息收入、理财收益 |
| 🏠 租金收入 | 房屋租金、车位租金 |
| 💝 退款收入 | 退货退款、报销到账 |
| 🎊 奖金中奖 | 中奖、竞赛奖金 |
| 🔄 其他收入 | 其他 |

---

## 预设数据 SQL

```sql
-- 一级分类
-- 一级分类（支出，type 默认为 'expense'）
INSERT INTO categories (id, family_id, parent_id, level, name, icon, color, type, sort_order) VALUES
(1,  NULL, NULL, 1, '餐饮',     '🍽️', '#FF6B6B', 'expense', 1),
(2,  NULL, NULL, 1, '交通出行', '🚗', '#4ECDC4', 'expense', 2),
(3,  NULL, NULL, 1, '服饰美妆', '👗', '#FF69B4', 'expense', 3),
(4,  NULL, NULL, 1, '住房',     '🏠', '#45B7D1', 'expense', 4),
(5,  NULL, NULL, 1, '休闲娱乐', '🎮', '#96CEB4', 'expense', 5),
(6,  NULL, NULL, 1, '医疗健康', '💊', '#FF8A80', 'expense', 6),
(7,  NULL, NULL, 1, '教育学习', '📚', '#7C4DFF', 'expense', 7),
(8,  NULL, NULL, 1, '养车',     '🚙', '#FFA726', 'expense', 8),
(9,  NULL, NULL, 1, '人情往来', '🤝', '#EC407A', 'expense', 9),
(10, NULL, NULL, 1, '母婴亲子', '👶', '#AB47BC', 'expense', 10),
(11, NULL, NULL, 1, '宠物',     '🐾', '#8D6E63', 'expense', 11),
(12, NULL, NULL, 1, '数码科技', '📱', '#29B6F6', 'expense', 12),
(13, NULL, NULL, 1, '金融理财', '💰', '#FFD700', 'expense', 13),
(14, NULL, NULL, 1, '差旅',     '🧳', '#26A69A', 'expense', 14),
(15, NULL, NULL, 1, '工作办公', '💼', '#78909C', 'expense', 15),
(16, NULL, NULL, 1, '其他',     '❓', '#BDBDBD', 'expense', 99);

-- 餐饮 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 1, 2, '早饭',     '☀️', 'expense', 1),
(NULL, 1, 2, '午饭',     '🌞', 'expense', 2),
(NULL, 1, 2, '晚饭',     '🌙', 'expense', 3),
(NULL, 1, 2, '下午茶',   '🫖', 'expense', 4),
(NULL, 1, 2, '夜宵', '🌃', 'expense', 5),
(NULL, 1, 2, '正餐聚餐', '🍜', 'expense', 6),
(NULL, 1, 2, '饮品', '🥤', 'expense', 7),
(NULL, 1, 2, '甜点零食', '🍰', 'expense', 8),
(NULL, 1, 2, '水果', '🍎', 'expense', 9),
(NULL, 1, 2, '食材采购', '🛒', 'expense', 10),
(NULL, 1, 2, '工作餐', '🍱', 'expense', 11);

-- 餐饮-早饭 三级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, (SELECT id FROM categories WHERE name='早饭' AND level=2), 3, '食堂', '🏫', 'expense', 1),
(NULL, (SELECT id FROM categories WHERE name='早饭' AND level=2), 3, '外卖', '🛵', 'expense', 2),
(NULL, (SELECT id FROM categories WHERE name='早饭' AND level=2), 3, '自己做', '🍳', 'expense', 3),
(NULL, (SELECT id FROM categories WHERE name='早饭' AND level=2), 3, '便利店', '🏪', 'expense', 4),
(NULL, (SELECT id FROM categories WHERE name='早饭' AND level=2), 3, '路边摊', '🛖', 'expense', 5);

-- 餐饮-午饭 三级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, (SELECT id FROM categories WHERE name='午饭' AND level=2), 3, '食堂', '🏫', 'expense', 1),
(NULL, (SELECT id FROM categories WHERE name='午饭' AND level=2), 3, '外卖', '🛵', 'expense', 2),
(NULL, (SELECT id FROM categories WHERE name='午饭' AND level=2), 3, '自己做', '🍳', 'expense', 3),
(NULL, (SELECT id FROM categories WHERE name='午饭' AND level=2), 3, '堂食', '🍽️', 'expense', 4),
(NULL, (SELECT id FROM categories WHERE name='午饭' AND level=2), 3, '便利店', '🏪', 'expense', 5),
(NULL, (SELECT id FROM categories WHERE name='午饭' AND level=2), 3, '快餐', '🍔', 'expense', 6);

-- 餐饮-下午茶 三级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, (SELECT id FROM categories WHERE name='下午茶' AND level=2), 3, '奶茶', '🧋', 'expense', 1),
(NULL, (SELECT id FROM categories WHERE name='下午茶' AND level=2), 3, '咖啡', '☕', 'expense', 2),
(NULL, (SELECT id FROM categories WHERE name='下午茶' AND level=2), 3, '甜点', '🧁', 'expense', 3),
(NULL, (SELECT id FROM categories WHERE name='下午茶' AND level=2), 3, '果汁', '🧃', 'expense', 4),
(NULL, (SELECT id FROM categories WHERE name='下午茶' AND level=2), 3, '茶饮', '🍵', 'expense', 5);

-- 餐饮-饮品 三级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, (SELECT id FROM categories WHERE name='饮品' AND level=2), 3, '奶茶', '🧋', 'expense', 1),
(NULL, (SELECT id FROM categories WHERE name='饮品' AND level=2), 3, '咖啡', '☕', 'expense', 2),
(NULL, (SELECT id FROM categories WHERE name='饮品' AND level=2), 3, '果汁', '🧃', 'expense', 3),
(NULL, (SELECT id FROM categories WHERE name='饮品' AND level=2), 3, '矿泉水', '💧', 'expense', 4),
(NULL, (SELECT id FROM categories WHERE name='饮品' AND level=2), 3, '碳酸饮料', '🥤', 'expense', 5),
(NULL, (SELECT id FROM categories WHERE name='饮品' AND level=2), 3, '茶', '🍵', 'expense', 6);

-- 餐饮-甜点零食 三级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, (SELECT id FROM categories WHERE name='甜点零食' AND level=2), 3, '蛋糕', '🎂', 'expense', 1),
(NULL, (SELECT id FROM categories WHERE name='甜点零食' AND level=2), 3, '面包', '🍞', 'expense', 2),
(NULL, (SELECT id FROM categories WHERE name='甜点零食' AND level=2), 3, '饼干', '🍪', 'expense', 3),
(NULL, (SELECT id FROM categories WHERE name='甜点零食' AND level=2), 3, '糖果', '🍬', 'expense', 4),
(NULL, (SELECT id FROM categories WHERE name='甜点零食' AND level=2), 3, '坚果', '🥜', 'expense', 5),
(NULL, (SELECT id FROM categories WHERE name='甜点零食' AND level=2), 3, '薯片', '🥔', 'expense', 6),
(NULL, (SELECT id FROM categories WHERE name='甜点零食' AND level=2), 3, '巧克力', '🍫', 'expense', 7);

-- 餐饮-水果 三级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, (SELECT id FROM categories WHERE name='水果' AND level=2), 3, '苹果', '🍎', 'expense', 1),
(NULL, (SELECT id FROM categories WHERE name='水果' AND level=2), 3, '香蕉', '🍌', 'expense', 2),
(NULL, (SELECT id FROM categories WHERE name='水果' AND level=2), 3, '葡萄', '🍇', 'expense', 3),
(NULL, (SELECT id FROM categories WHERE name='水果' AND level=2), 3, '西瓜', '🍉', 'expense', 4),
(NULL, (SELECT id FROM categories WHERE name='水果' AND level=2), 3, '车厘子', '🍒', 'expense', 5),
(NULL, (SELECT id FROM categories WHERE name='水果' AND level=2), 3, '其他水果', '🥝', 'expense', 6);

-- 餐饮-食材采购 三级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, (SELECT id FROM categories WHERE name='食材采购' AND level=2), 3, '蔬菜', '🥬', 'expense', 1),
(NULL, (SELECT id FROM categories WHERE name='食材采购' AND level=2), 3, '肉类', '🥩', 'expense', 2),
(NULL, (SELECT id FROM categories WHERE name='食材采购' AND level=2), 3, '海鲜', '🦐', 'expense', 3),
(NULL, (SELECT id FROM categories WHERE name='食材采购' AND level=2), 3, '蛋奶', '🥚', 'expense', 4),
(NULL, (SELECT id FROM categories WHERE name='食材采购' AND level=2), 3, '粮油', '🌾', 'expense', 5),
(NULL, (SELECT id FROM categories WHERE name='食材采购' AND level=2), 3, '调味品', '🧂', 'expense', 6),
(NULL, (SELECT id FROM categories WHERE name='食材采购' AND level=2), 3, '半成品', '🥟', 'expense', 7);

-- 餐饮-正餐聚餐 三级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, (SELECT id FROM categories WHERE name='正餐聚餐' AND level=2), 3, '中餐', '🥘', 'expense', 1),
(NULL, (SELECT id FROM categories WHERE name='正餐聚餐' AND level=2), 3, '西餐', '🥩', 'expense', 2),
(NULL, (SELECT id FROM categories WHERE name='正餐聚餐' AND level=2), 3, '日料', '🍣', 'expense', 3),
(NULL, (SELECT id FROM categories WHERE name='正餐聚餐' AND level=2), 3, '韩餐', '🍜', 'expense', 4),
(NULL, (SELECT id FROM categories WHERE name='正餐聚餐' AND level=2), 3, '火锅', '🍲', 'expense', 5),
(NULL, (SELECT id FROM categories WHERE name='正餐聚餐' AND level=2), 3, '烧烤', '🍖', 'expense', 6),
(NULL, (SELECT id FROM categories WHERE name='正餐聚餐' AND level=2), 3, '自助餐', '🍽️', 'expense', 7);

-- 交通出行 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 2, 2, '打车', '🚕', 'expense', 1),
(NULL, 2, 2, '公共交通', '🚇', 'expense', 2),
(NULL, 2, 2, '火车高铁', '🚄', 'expense', 3),
(NULL, 2, 2, '飞机', '✈️', 'expense', 4),
(NULL, 2, 2, '长途汽车', '🚌', 'expense', 5),
(NULL, 2, 2, '自驾加油', '⛽', 'expense', 6),
(NULL, 2, 2, '停车费', '🅿️', 'expense', 7),
(NULL, 2, 2, '共享单车', '🛵', 'expense', 8),
(NULL, 2, 2, '车辆保养', '🔧', 'expense', 9),
(NULL, 2, 2, '车险', '🛡️', 'expense', 10),
(NULL, 2, 2, '车辆费用', '📋', 'expense', 11),
(NULL, 2, 2, '出行服务', '🛤️', 'expense', 12);

-- 交通-打车 三级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, (SELECT id FROM categories WHERE name='打车' AND level=2), 3, '滴滴', '🚗', 'expense', 1),
(NULL, (SELECT id FROM categories WHERE name='打车' AND level=2), 3, '高德打车', '🗺️', 'expense', 2),
(NULL, (SELECT id FROM categories WHERE name='打车' AND level=2), 3, '曹操出行', '🟢', 'expense', 3),
(NULL, (SELECT id FROM categories WHERE name='打车' AND level=2), 3, 'T3出行', '🔵', 'expense', 4),
(NULL, (SELECT id FROM categories WHERE name='打车' AND level=2), 3, '出租车', '🚕', 'expense', 5);

-- 交通-公共交通 三级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, (SELECT id FROM categories WHERE name='公共交通' AND level=2), 3, '地铁', '🚇', 'expense', 1),
(NULL, (SELECT id FROM categories WHERE name='公共交通' AND level=2), 3, '公交', '🚌', 'expense', 2),
(NULL, (SELECT id FROM categories WHERE name='公共交通' AND level=2), 3, '轮渡', '⛴️', 'expense', 3),
(NULL, (SELECT id FROM categories WHERE name='公共交通' AND level=2), 3, '城际铁路', '🚆', 'expense', 4);

-- 交通-火车高铁 三级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, (SELECT id FROM categories WHERE name='火车高铁' AND level=2), 3, '高铁', '🚄', 'expense', 1),
(NULL, (SELECT id FROM categories WHERE name='火车高铁' AND level=2), 3, '动车', '🚅', 'expense', 2),
(NULL, (SELECT id FROM categories WHERE name='火车高铁' AND level=2), 3, '普通火车', '🚂', 'expense', 3),
(NULL, (SELECT id FROM categories WHERE name='火车高铁' AND level=2), 3, '城际', '🚆', 'expense', 4);

-- 服饰美妆 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 3, 2, '男装', '👔', 'expense', 1),
(NULL, 3, 2, '女装', '👗', 'income', 2),
(NULL, 3, 2, '鞋子', '👟', 'income', 3),
(NULL, 3, 2, '包包', '👜', 'income', 4),
(NULL, 3, 2, '饰品', '💍', 'income', 5),
(NULL, 3, 2, '美妆护肤', '💄', 'income', 6),
(NULL, 3, 2, '美容美发', '💇', 'income', 7),
(NULL, 3, 2, '日用品', '🧴', 'income', 8);

-- 住房 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 4, 2, '房租/房贷', '🏷️', 'income', 1),
(NULL, 4, 2, '水电燃气', '💡', 'income', 2),
(NULL, 4, 2, '通讯网络', '📶', 'income', 3),
(NULL, 4, 2, '物业费', '🏠', 'income', 4),
(NULL, 4, 2, '维修维护', '🔧', 'income', 5),
(NULL, 4, 2, '家居用品', '🛋️', 'income', 6),
(NULL, 4, 2, '家政服务', '🧹', 'income', 7),
(NULL, 4, 2, '装修', '🏗️', 'income', 8);

-- 休闲娱乐 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 5, 2, '影视', '🎬', 'income', 1),
(NULL, 5, 2, '音乐', '🎵', 'income', 2),
(NULL, 5, 2, '阅读', '📚', 'income', 3),
(NULL, 5, 2, '游戏', '🎮', 'income', 4),
(NULL, 5, 2, '运动健身', '🏃', 'income', 5),
(NULL, 5, 2, '休闲活动', '🎳', 'income', 6),
(NULL, 5, 2, '旅游景点', '🎪', 'income', 7),
(NULL, 5, 2, '摄影', '📷', 'income', 8),
(NULL, 5, 2, '兴趣爱好', '🎨', 'income', 9);

-- 医疗健康 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 6, 2, '就医', '🏥', 'income', 1),
(NULL, 6, 2, '药品', '💊', 'income', 2),
(NULL, 6, 2, '口腔', '🦷', 'income', 3),
(NULL, 6, 2, '眼科', '👁️', 'income', 4),
(NULL, 6, 2, '疫苗体检', '💉', 'income', 5),
(NULL, 6, 2, '保健调理', '🧘', 'income', 6),
(NULL, 6, 2, '健身器材', '🏋️', 'income', 7);

-- 教育学习 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 7, 2, '学费', '📖', 'income', 1),
(NULL, 7, 2, '考试考证', '📝', 'income', 2),
(NULL, 7, 2, '文具用品', '📓', 'income', 3),
(NULL, 7, 2, '在线课程', '💻', 'income', 4),
(NULL, 7, 2, '辅导培训', '🎓', 'income', 5),
(NULL, 7, 2, '教材教辅', '📖', 'income', 6);

-- 养车 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 8, 2, '加油', '⛽', 'income', 1),
(NULL, 8, 2, '保养维修', '🔧', 'income', 2),
(NULL, 8, 2, '洗车美容', '🚿', 'income', 3),
(NULL, 8, 2, '停车', '🅿️', 'income', 4),
(NULL, 8, 2, '过路费', '🛣️', 'income', 5),
(NULL, 8, 2, '车辆行政', '📋', 'income', 6),
(NULL, 8, 2, '车险', '🛡️', 'income', 7);

-- 人情往来 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 9, 2, '礼物', '🎁', 'income', 1),
(NULL, 9, 2, '红包', '💰', 'income', 2),
(NULL, 9, 2, '婚礼', '🎊', 'income', 3),
(NULL, 9, 2, '生日', '🎂', 'income', 4),
(NULL, 9, 2, '节日', '🏮', 'income', 5),
(NULL, 9, 2, '请客', '🤝', 'income', 6),
(NULL, 9, 2, '探望', '💐', 'income', 7);

-- 母婴亲子 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 10, 2, '奶粉辅食', '🍼', 'income', 1),
(NULL, 10, 2, '玩具', '🧸', 'income', 2),
(NULL, 10, 2, '婴儿用品', '👶', 'income', 3),
(NULL, 10, 2, '童装', '👗', 'income', 4),
(NULL, 10, 2, '早教', '📚', 'income', 5),
(NULL, 10, 2, '亲子活动', '🎠', 'income', 6);

-- 宠物 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 11, 2, '宠物食品', '🍖', 'income', 1),
(NULL, 11, 2, '宠物医疗', '🏥', 'income', 2),
(NULL, 11, 2, '宠物用品', '🧴', 'income', 3),
(NULL, 11, 2, '宠物服务', '✂️', 'income', 4);

-- 数码科技 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 12, 2, '手机', '📱', 'income', 1),
(NULL, 12, 2, '电脑', '💻', 'income', 2),
(NULL, 12, 2, '数码配件', '🎧', 'income', 3),
(NULL, 12, 2, '办公设备', '🖨️', 'income', 4),
(NULL, 12, 2, '家电', '📺', 'income', 5),
(NULL, 12, 2, '游戏设备', '🎮', 'income', 6);

-- 金融理财 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 13, 2, '银行费用', '💳', 'income', 1),
(NULL, 13, 2, '投资', '📈', 'income', 2),
(NULL, 13, 2, '贷款利息', '🏦', 'income', 3),
(NULL, 13, 2, '汇率换汇', '💱', 'income', 4),
(NULL, 13, 2, '保险', '📊', 'income', 5),
(NULL, 13, 2, '税费', '💸', 'income', 6);

-- 差旅 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 14, 2, '住宿', '🏨', 'income', 1),
(NULL, 14, 2, '差旅餐饮', '🍽️', 'income', 2),
(NULL, 14, 2, '差旅交通', '🚕', 'income', 3),
(NULL, 14, 2, '差旅其他', '📋', 'income', 4);

-- 工作办公 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 15, 2, '办公用品', '🖊️', 'income', 1),
(NULL, 15, 2, '工作餐', '🍽️', 'income', 2),
(NULL, 15, 2, '工作出行', '🚕', 'income', 3),
(NULL, 15, 2, '通讯办公', '📱', 'income', 4),
(NULL, 15, 2, '职业发展', '🎓', 'income', 5);

-- 其他 二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 16, 2, '未分类', '❓', 'income', 1),
(NULL, 16, 2, '转账', '🔄', 'income', 2),
(NULL, 16, 2, '手续费', '💸', 'income', 3),
(NULL, 16, 2, '彩票', '🎰', 'expense', 4),
(NULL, 16, 2, '慈善捐赠', '🙏', 'expense', 5),
(NULL, 16, 2, '罚款', '⚖️', 'expense', 6),
(NULL, 16, 2, '快递物流', '📦', 'expense', 7);

-- 收入分类（注意：必须指定 type='income'，否则默认为 expense）
INSERT INTO categories (id, family_id, parent_id, level, name, icon, color, type, sort_order) VALUES
(100, NULL, NULL, 1, '工资薪酬',   '💰', '#4CAF50', 'income', 1),
(101, NULL, NULL, 1, '副业收入',   '💵', '#66BB6A', 'income', 2),
(102, NULL, NULL, 1, '红包收入',   '🎁', '#FF7043', 'income', 3),
(103, NULL, NULL, 1, '转账收入',   '💸', '#42A5F5', 'income', 4),
(104, NULL, NULL, 1, '投资收益',   '📈', '#FFD700', 'income', 5),
(105, NULL, NULL, 1, '租金收入',   '🏠', '#78909C', 'income', 6),
(106, NULL, NULL, 1, '退款收入',   '💝', '#AB47BC', 'income', 7),
(107, NULL, NULL, 1, '奖金中奖',   '🎊', '#FF6B6B', 'income', 8),
(108, NULL, NULL, 1, '其他收入',   '🔄', '#BDBDBD', 'income', 99);

-- 收入二级分类
INSERT INTO categories (family_id, parent_id, level, name, icon, type, sort_order) VALUES
(NULL, 100, 2, '月薪',     '💵', 'income', 1),
(NULL, 100, 2, '年终奖',   '🎊', 'income', 2),
(NULL, 100, 2, '绩效奖金', '🏆', 'income', 3),
(NULL, 100, 2, '加班费',   '⏰', 'income', 4),
(NULL, 100, 2, '补贴津贴', '📋', 'income', 5),
(NULL, 101, 2, '兼职',     '💼', 'income', 1),
(NULL, 101, 2, 'freelance','💻', 'income', 2),
(NULL, 101, 2, '稿费',     '📝', 'income', 3),
(NULL, 101, 2, '咨询费',   '🗣️', 'income', 4);
```

---

## 统计数据

| 一级分类 | 二级分类数 | 三级分类数 |
|---------|:---------:|:---------:|
| 🍽️ 餐饮 | 11 | 44 |
| 🚗 交通出行 | 12 | 18 |
| 👕 服饰美妆 | 8 | - |
| 🏠 住房 | 8 | - |
| 🎮 休闲娱乐 | 9 | - |
| 💊 医疗健康 | 7 | - |
| 📚 教育学习 | 6 | - |
| 🚗 养车 | 7 | - |
| 🤝 人情往来 | 7 | - |
| 👶 母婴亲子 | 6 | - |
| 🐾 宠物 | 4 | - |
| 📱 数码科技 | 6 | - |
| 💰 金融理财 | 6 | - |
| 🧳 差旅 | 4 | - |
| 💼 工作办公 | 5 | - |
| ❓ 其他 | 7 | - |
| **合计** | **113** | **62+** |

> 高频使用分类（餐饮、交通）已细化到三级，低频分类保留二级，用户可自行扩展三级。
